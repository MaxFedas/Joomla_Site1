...
$googlefont     = $config->get('googlefont');
$fontweight     = $config->get('fontweight');
$googlefontH    = $config->get('googlefontH');
$fontweightH    = $config->get('fontweightH');
...
if ($googlefont != ''):
	if ($fontweight != ''):
		$doc->addStyleSheet('https://fonts.googleapis.com/css?family=' . $googlefont . ':' . $fontweight );
		$font = 'body{' . 'font-family:' . $googlefont . ',Helvetica,Arial,FreeSans,sans-serif;font-weight:' . $fontweight . '}';
		$doc->addStyleDeclaration($font);	
	else:
		$font = 'body{' . 'font-family:Helvetica,Arial,FreeSans,sans-serif;font-weight:normal}';
		$doc->addStyleDeclaration($font);
	endif;
endif;
if ($googlefontH != ''):
	if ($fontweightH != ''):
		$doc->addStyleSheet('https://fonts.googleapis.com/css?family=' . $googlefontH . ':' . $fontweightH );
		$fontH = 'h1,h2,h3,h4,h5,h6{' . 'font-family:' . $googlefontH . ',Georgia,Verdana,sans-serif;font-weight:' . $fontweightH . '}';
		$doc->addStyleDeclaration($fontH);
	else:
		$fontH = 'h1,h2,h3,h4,h5,h6{' . 'font-family:"Helvetica Neue",Helvetica,Arial,FreeSans,sans-serif;font-weight:bold}';
		$doc->addStyleDeclaration($fontH);
	endif;
endif;
...
// add scripts
if ($this['config']->get('animate', true)) :
	$this['asset']->addFile('js', 'js:jquery.appear.js');
	$this['asset']->addFile('js', 'js:theme.animate.js');
endif;
...
$this['asset']->addFile('js', 'js:theme.bootstrap.tooltip.js');
